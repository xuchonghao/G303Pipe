...
public class StateSpaceGenerator {
		...
		if (isTangible(pnmlData, currentMarking)) {
			...
			while ((!vanishingStates.isEmpty()) && (attempts != MAX_TRIES)) {
				...
				for(int i = 0; i < v.getState().length; i++){
					...
					MarkingView m = new MarkingView(pnmlData.getTokenViews().getFirst(), v.getState()[i]);
					mlist.add(m);
					state[i]= mlist;
				}
	...
	private static boolean isTangible(PetriNetView pnmlData, State marking) {
		...
		for(int i = 0; i < marking.getState().length; i++){
			LinkedList<MarkingView> mlist = new LinkedList<MarkingView>();
			MarkingView m = new MarkingView(pnmlData.getTokenViews().getFirst(), marking.getState()[i]);
			mlist.add(m);
			state[i]= mlist;
		}
     ...
     * @return
	 */
	private static int fire(PetriNetView pnmlData, State vs, Stack succ) {
		...
		for(int i = 0; i < vs.getState().length; i++){
			LinkedList<MarkingView> mlist = new LinkedList<MarkingView>();
			MarkingView m = new MarkingView(pnmlData.getTokenViews().getFirst(), vs.getState()[i]);
			mlist.add(m);
			state[i]= mlist;
		}
		boolean[] enabledTransitions = pnmlData
	...
	private static double rate(PetriNetView pnmlData, State s, State sprime) {
		...
		int[][] incidenceMatrix = pnmlData.getTokenViews().
		...
		for(int i = 0; i < marking1.length; i++){
			LinkedList<MarkingView> mlist = new LinkedList<MarkingView>();
			MarkingView m = new MarkingView(pnmlData.getTokenViews().getFirst(), marking1[i]);
			mlist.add(m);
			state[i]= mlist;
		}
		...
		for (int i = 0; i < transCount; i++) {
			if ((matchingTransition[i])
					&& (marking1EnabledTransitions[i])) {
				candidateTransitionWeighting += pnmlData.getTransitionViews()[i]
						.getRate();
			}
		}
	...
	private static double prob(PetriNetView pnmlData, State s, State sprime,
		...
		int[] marking2 = sprime.getState();
		int markSize = marking1.length;
		int[][] incidenceMatrix = pnmlData.getTokenViews().getFirst().
		...
		double candidateTransitionWeighting = pnmlData.getTransitionViews()[firedTransition]
				.getRate();
